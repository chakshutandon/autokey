...
def rotate_string_const(string, n, alphabet, forward=True):
    ...
    def func(char): return rotate_char(char, n, alphabet)
    m = map(func, string)
    return "".join(m)
...
def __rotate_string_with_key(string, key, alphabet, forward=True):
    ...
    def func(i, n): return rotate_char(string[i], n, alphabet, forward)
...
def decode_string_with_plaintext_autokey(ciphertext, key, alphabet):
    ...
    while (len(plaintext) < len(ciphertext)):
        _key = key + plaintext
        plaintext = __rotate_string_with_key(
            ciphertext, _key, alphabet, forward=False)
    return plaintext
